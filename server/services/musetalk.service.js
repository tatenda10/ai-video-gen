...
export class MuseTalkService {
  constructor(config) {
    this.config = config;
    this.apiUrl = config.musetalk.apiUrl;
  }
  ...
  async generateTalkingHead(imagePath, audioPath, sceneId, jobId, tempDir) {
    try {
      log.info(`Generating talking head for scene ${sceneId} using MuseTalk...`);
      ...
      // Check if MuseTalk API is available
      try {
        log.info(`Checking MuseTalk API health at ${this.apiUrl}/health...`);
        const healthResponse = await axios.get(`${this.apiUrl}/health`, { timeout: 10000 });
        log.info(`MuseTalk API is healthy:`, healthResponse.data);
      } catch (error) {
        log.error(`MuseTalk API health check failed:`, error.message);
        throw new Error(`MuseTalk API not available at ${this.apiUrl}. Make sure the Flask server is running. Error: ${error.message}`);
      }
      ...
      // Optional parameters
      formData.append('bbox_shift', '0'); // Default, can be adjusted
      formData.append('use_float16', 'true'); // Faster inference
      formData.append('fps', '25'); // Video FPS
      formData.append('gpu_id', '0'); // GPU ID
      ...
      log.error(`MuseTalk generation failed for scene ${sceneId}:`, error.message);
      ...
      if (error.code === 'ETIMEDOUT' || error.message.includes('timeout')) {
        throw new Error(`MuseTalk generation timed out for scene ${sceneId}. MuseTalk processing may be taking too long.`);
      }
      
      throw new Error(`MuseTalk generation failed for scene ${sceneId}: ${error.message}`);
    }
  }
